...
public class SqLiteDatabase extends Database {
    ...
    public void addServerLink(@NotNull String masterServer, @NotNull String slaveServer) {
        try (PreparedStatement statement = connection.prepareStatement(formatStatementTables("""
                INSERT OR REPLACE INTO `%server_links_table%` (`master_server`, `slave_server`) VALUES (?, ?);"""))) {
            statement.setString(1, masterServer);
    ...
    public void setWarpPermission(@NotNull String warpName, @NotNull String permission) {
        try (PreparedStatement statement = connection.prepareStatement(formatStatementTables("""
                INSERT OR REPLACE INTO `%warp_permissions_table%` (`warp_name`, `permission`) VALUES (?, ?);"""))) {
            statement.setString(1, warpName);
    ...
    public void setServerPermission(@NotNull String serverName, @NotNull String permission) {
        try (PreparedStatement statement = connection.prepareStatement(formatStatementTables("""
                INSERT OR REPLACE INTO `%server_permissions_table%` (`server_name`, `permission`) VALUES (?, ?);"""))) {
            statement.setString(1, serverName);
            statement.setString(2, permission);
            statement.executeUpdate();
        } catch (SQLException e) {
    ...
    public int renameServer(@NotNull String oldName, @NotNull String newName) {
        try {
            ...
            try {
                ...
                try (PreparedStatement stmt = connection.prepareStatement(formatStatementTables(
                        "UPDATE `%user_preferences_table%` SET `master_server` = ? WHERE `master_server` = ?;"))) {
                    stmt.setString(1, newName);
                    stmt.setString(2, oldName);
                    updated += stmt.executeUpdate();
                }
